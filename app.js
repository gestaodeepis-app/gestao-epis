...
function renderUsers(){
  ...
  const q=($("userSearch")?.value||"").trim().toLowerCase(),status=$("userStatusFilter")?.value||userStatusFilter||"",list=db.users.filter(u=>(!dashboardPlantId||u.plantId===dashboardPlantId||u.plantId==="TODAS")).filter(u=>(!status||u.status===status)&&(!q||(u.name||"").toLowerCase().includes(q)||(u.email||"").toLowerCase().includes(q))).sort((a,b)=>(a.status==="PENDENTE"?0:a.status==="BLOQUEADO"?2:1)-(b.status==="PENDENTE"?0:b.status==="BLOQUEADO"?2:1)||(a.name||a.email||"").localeCompare(b.name||b.email||""));
