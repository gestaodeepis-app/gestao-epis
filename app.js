...
function renderPlants(){$("plantsGrid").innerHTML=db.plants.map(p=>`<div class="plant-card"><strong>${p.name}</strong><small>Código: ${p.code}</small><small>${p.address||"Endereço não informado"}</small></div>`).join("")||'<div class="empty-state">Nenhuma usina cadastrada.</div>'}
...
$("plantForm").onsubmit=async e=>{
  e.preventDefault();
  try{await addDoc(collection(firestore,"plants"),{name:$("uName").value.trim(),code:$("uCode").value.trim(),address:$("uAddress").value.trim(),createdAt:serverTimestamp(),createdBy:auth.currentUser.uid});await logAudit("CADASTRO_USINA",$("uName").value.trim());e.target.reset();closeModal("plantModal");await loadAll()}
