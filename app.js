...
function stopPendingWatcher(){if(pendingWatcher){clearInterval(pendingWatcher);pendingWatcher=null}}
function showPendingAccess(data,state="PENDENTE"){
  $("loginScreen").classList.add("hidden");$("accessPendingScreen").classList.remove("hidden");$("pendingUserName").textContent=data?.name||"Usuário";$("pendingUserEmail").textContent=data?.email||auth.currentUser?.email||"-";$("pendingEnterBtn").classList.add("hidden");
  ...
  icon.classList.add("waiting");icon.textContent="⌛";$("pendingStateKicker").textContent="SOLICITAÇÃO ENVIADA";$("pendingStateTitle").textContent="Aguardando aprovação";$("pendingStateText").textContent="Seu cadastro foi recebido. O Proprietário do Sistema precisa definir sua função e usina antes de liberar o acesso.";$("pendingLastCheck").textContent="Verificando aprovação automaticamente...";startPendingWatcher();
}
function showApprovedAccess(data){stopPendingWatcher();$("loginScreen").classList.add("hidden");$("accessPendingScreen").classList.remove("hidden");$("pendingUserName").textContent=data?.name||"Usuário";$("pendingUserEmail").textContent=data?.email||auth.currentUser?.email||"-";const icon=$("pendingStateIcon");icon.className="pending-state-icon approved";icon.textContent="✓";$("pendingStateKicker").textContent="ACESSO APROVADO";$("pendingStateTitle").textContent="Seu acesso foi liberado";$("pendingStateText").textContent=`Função: ${humanRole(data.role)} • ${data.plantId==="TODAS"?"Todas as usinas":plantName(data.plantId)}.`;$("pendingLastCheck").textContent="Clique abaixo para entrar no sistema.";$("pendingEnterBtn").classList.remove("hidden")}
async function startPendingWatcher(){stopPendingWatcher();const check=async()=>{if(!auth.currentUser)return;try{const s=await getDoc(doc(firestore,"users",auth.currentUser.uid));if(!s.exists())return;const d={uid:s.id,email:auth.currentUser.email,...s.data()};$("pendingLastCheck").textContent=`Última verificação: ${new Date().toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit",second:"2-digit"})}`;if(d.status==="ATIVO")showApprovedAccess(d);else if(d.status==="BLOQUEADO")showPendingAccess(d,"BLOQUEADO")}catch(err){console.debug("Aguardando aprovação:",err)}};await check();pendingWatcher=setInterval(check,5000)}
async function enterApprovedSystem(){if(!auth.currentUser)return;const s=await getDoc(doc(firestore,"users",auth.currentUser.uid));if(!s.exists())return;const d={uid:auth.currentUser.uid,email:auth.currentUser.email,...s.data()};if(d.status!=="ATIVO"){showPendingAccess(d,d.status);return}profile=d;$("accessPendingScreen").classList.add("hidden");updateProfileUI();applyPermissions();startPresence();await loadAll();$("loginScreen").classList.add("hidden");handleEmployeeDeepLink()}
...
$("pendingLogoutBtn").addEventListener("click",async()=>{stopPendingWatcher();await signOut(auth);$("accessPendingScreen").classList.add("hidden");$("loginScreen").classList.remove("hidden");setAuthMode("login")});
...
$("logoutBtn").addEventListener("click",()=>{if(ownerUsersRefreshTimer)clearInterval(ownerUsersRefreshTimer);stopPendingWatcher();signOut(auth)});

onAuthStateChanged(auth,async user=>{
  if(registrationInProgress)return;
  if(!user){stopPendingWatcher();profile=null;$("accessPendingScreen").classList.add("hidden");$("loginScreen").classList.remove("hidden");return}
  try{
  ...
  try{await loadAll();$("loginScreen").classList.add("hidden");handleEmployeeDeepLink()}catch(err){console.error("Falha ao carregar dados:",err);$("loginScreen").classList.add("hidden");showToast("Você continua conectado. Alguns dados não puderam ser atualizados agora.","error")}
});
...
async function loadAll(){
  ...
  pendingLogoDataUrl=db.settings.logoDataUrl||null;applyBranding();updateProfileUI();refresh();startOwnerUsersRefresh();
}
...
function startOwnerUsersRefresh(){if(ownerUsersRefreshTimer)clearInterval(ownerUsersRefreshTimer);if(!isOwner())return;ownerUsersRefreshTimer=setInterval(refreshOwnerUsers,30000)}
...
$("userApprovalForm").onsubmit=async e=>{e.preventDefault();if(!isOwner())return;const id=$("approvalUserId").value,u=db.users.find(x=>x.id===id),role=$("approvalRole").value,plantId=$("approvalPlant").value;if(!u||!role||!plantId){showToast("Selecione a função e a usina.","error");return}try{const data={status:"ATIVO",role,plantId,isOwner:false,approvedAt:serverTimestamp(),approvedBy:auth.currentUser.uid,updatedAt:serverTimestamp(),updatedBy:auth.currentUser.uid};await updateDoc(doc(firestore,"users",id),data);const i=db.users.findIndex(x=>x.id===id);if(i>=0)db.users[i]={...db.users[i],...data,approvedAt:new Date(),updatedAt:new Date()};logAudit("APROVACAO_USUARIO",`${u.name||u.email} • ${humanRole(role)} • ${plantId==="TODAS"?"Todas as usinas":plantName(plantId)}`);closeModal("userApprovalModal");renderUsers();renderNotifications();renderPendingUsersBadge();showToast(`Acesso de ${u.name||u.email} aprovado.`)}catch(err){console.error(err);showToast("Não foi possível aprovar o usuário.","error")}};
