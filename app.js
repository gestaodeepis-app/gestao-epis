...
async function syncSystemData(){
  ...
  try{
    ...
    if(currentView==="auditoria"&&isAdmin())await ensureAuditLoaded(true);
...
function renderUsers(){
  ...
  const q=($("userSearch")?.value||"").trim().toLowerCase(),status=$("userStatusFilter")?.value||userStatusFilter||"",list=db.users.filter(u=>(u.status==="PENDENTE"||!dashboardPlantId||u.plantId===dashboardPlantId||u.plantId==="TODAS")).filter(u=>(!status||u.status===status)&&(!q||(u.name||"").toLowerCase().includes(q)||(u.email||"").toLowerCase().includes(q))).sort((a,b)=>(a.status==="PENDENTE"?0:a.status==="BLOQUEADO"?2:1)-(b.status==="PENDENTE"?0:b.status==="BLOQUEADO"?2:1)||(a.name||a.email||"").localeCompare(b.name||b.email||""));
...
function populateDashboardFilters(){
  ...
  if(pid){
    ...
    if(mobile){mobile.innerHTML=options;mobile.value=pid;mobile.disabled=true}
    ...
    if($("mobileDashboardPlantWrap"))$("mobileDashboardPlantWrap").classList.add("locked-plant");
    return;
  }
  ...
  if(mobile){mobile.disabled=false;mobile.innerHTML=options;mobile.value=dashboardPlantId}
  ...
  if($("mobileDashboardPlantWrap"))$("mobileDashboardPlantWrap").classList.remove("locked-plant");
}
function applyGlobalPlantFilter(value){
  dashboardPlantId=assignedPlantId()||value||"";
  ...
  if($("epiPlantFilter")&&!assignedPlantId())$("epiPlantFilter").value=dashboardPlantId;
  if($("reportPlant")&&!assignedPlantId())$("reportPlant").value=dashboardPlantId;
  movementPage=1;
...
function openUserEdit(id){
  ...
  $("editUserId").value=id;$("editUserName").value=u.name||"";
  ...
  if($("userEditModalSubtitle"))$("userEditModalSubtitle").textContent=own?"Atualize seu nome e foto de identificação.":"Atualize o nome e a foto deste usuário.";
...
$("userEditForm").onsubmit=async e=>{
  e.preventDefault();
  try{
    const id=$("editUserId").value,name=$("editUserName").value.trim();
    const own=id===auth.currentUser?.uid;
    if(!own&&!isOwner()){userAlert("Você só pode editar o próprio perfil.");return}
    if(name.length<3){userAlert("Informe um nome válido.");return}
    const data={name,photoUrl:pendingUserPhotoDataUrl||"",updatedAt:serverTimestamp(),updatedBy:auth.currentUser.uid};
    await updateDoc(doc(firestore,"users",id),data);
    if(own){
      profile={...profile,...data,name,photoUrl:pendingUserPhotoDataUrl||""};
      updateProfileUI();
    }
    const i=db.users.findIndex(x=>x.id===id);
    if(i>=0)db.users[i]={...db.users[i],...data,name,photoUrl:pendingUserPhotoDataUrl||""};
    await logAudit(own?"EDICAO_PROPRIO_PERFIL":"EDICAO_USUARIO",name);
    closeModal("userEditModal");renderUsers();
    showToast(own?"Perfil atualizado com sucesso.":"Usuário atualizado.");
  }catch(err){
    console.error(err);userAlert("Não foi possível atualizar o perfil.");
  }
};
...
async function openNotificationTarget(key){
  ...
  if(target.kind==="epi"){
    if(!assignedPlantId())applyGlobalPlantFilter(target.plantId||"");
    navTo("epis");
