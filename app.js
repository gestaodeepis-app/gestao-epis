...
async function shareSystem(){
  const shareData={
    ...
    text:"Acesse o Sistema de Gestão de EPIs.",
    url:"https://gestaodeepis-app.github.io/gestao-epis/"
  };
  try{
    if(navigator.share){
    ...
    if(err?.name!=="AbortError"){
      try{await navigator.clipboard.writeText(shareData.url);showToast("Link do sistema copiado.")}catch(_){showToast("Não foi possível compartilhar o link.","error")}
    }
  }
}
