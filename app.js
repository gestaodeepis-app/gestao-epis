...
async function removeSelectedAudits(){
  ...
  try{await deleteAuditIds(ids);showToast(`${ids.length} registro(s) removido(s).`)}catch(err){console.error(err);alert("Não foi possível remover os registros selecionados.")}
}
window.removeSelectedAudits=removeSelectedAudits;
async function removeAllAudits(){
  if(!isOwner()||!db.audit.length)return;
  
  try{const ids=db.audit.map(x=>x.id);await deleteAuditIds(ids);showToast("Log de auditoria limpo.")}catch(err){console.error(err);alert("Não foi possível limpar o log de auditoria.")}
}
...
$("plantForm").onsubmit=async e=>{e.preventDefault();try{const name=$("uName").value.trim(),ref=await addDoc(collection(firestore,"plants"),{name,createdAt:serverTimestamp(),createdBy:auth.currentUser.uid});db.plants.push({id:ref.id,name});logAudit("CADASTRO_USINA",name);e.target.reset();closeModal("plantModal");refresh();showToast("Usina cadastrada com sucesso.")}catch(err){console.error(err);alert("Não foi possível cadastrar a usina.")}};
...
}catch(err){console.error(err);alert("Não foi possível salvar o colaborador.")}};
...
$("userEditForm").onsubmit=async e=>{e.preventDefault();try{const id=$("editUserId").value,name=$("editUserName").value.trim();await updateDoc(doc(firestore,"users",id),{name,photoUrl:pendingUserPhotoDataUrl||"",updatedAt:serverTimestamp(),updatedBy:auth.currentUser.uid});await logAudit("EDICAO_USUARIO",name);if(id===auth.currentUser.uid){profile.name=name;profile.photoUrl=pendingUserPhotoDataUrl||"";updateProfileUI()}const i=db.users.findIndex(x=>x.id===id);if(i>=0)db.users[i]={...db.users[i],name,photoUrl:pendingUserPhotoDataUrl||""};closeModal("userEditModal");renderUsers();showToast("Usuário atualizado.")}catch(err){console.error(err);alert("Não foi possível atualizar o usuário.")}};

async function removeMovement(id){if(!canDeleteOperational())return;const moveRef=doc(firestore,"movements",id);try{await runTransaction(firestore,async tx=>{const ms=await tx.get(moveRef);if(!ms.exists())return;const m=ms.data(),er=doc(firestore,"epis",m.epiId),es=await tx.get(er);if(es.exists()){const current=stockQty(es.data()),qty=+m.qty||0;let next=current;if(m.type==="Entrega")next=current+qty;else if(m.type==="Devolução"&&(m.returnDisposition||"REESTOQUE")==="REESTOQUE"){if(current<qty)throw new Error("ESTOQUE_INCONSISTENTE");next=current-qty}if(next!==current)tx.update(er,{stockQty:next,updatedAt:serverTimestamp(),updatedBy:auth.currentUser.uid})}tx.delete(moveRef)});logAudit("REMOCAO_MOVIMENTACAO",id);await reloadOperationalData()}catch(err){console.error(err);alert(err.message==="ESTOQUE_INCONSISTENTE"?"Não é possível remover esta devolução porque parte desse estoque já foi utilizado.":"Não foi possível remover a movimentação.")}}window.removeMovement=removeMovement;
async function removeEmployee(id){if(!canDeleteOperational())return;if(db.movements.some(m=>m.employeeId===id)){alert("Este colaborador possui movimentações. Remova as movimentações relacionadas antes.");return}const e=emp(id);try{await deleteDoc(doc(firestore,"employees",id));logAudit("REMOCAO_COLABORADOR",e?.name||id);db.employees=db.employees.filter(x=>x.id!==id);refresh();showToast("Colaborador removido.")}catch(err){console.error(err);alert("Não foi possível remover o colaborador.")}}window.removeEmployee=removeEmployee;
async function removeEpi(id){if(!canDeleteOperational())return;if(db.movements.some(m=>m.epiId===id)){alert("Este EPI possui movimentações. Remova as movimentações relacionadas antes.");return}const p=epi(id);try{await deleteDoc(doc(firestore,"epis",id));logAudit("REMOCAO_EPI",p?.name||id);db.epis=db.epis.filter(x=>x.id!==id);refresh();showToast("EPI removido.")}catch(err){console.error(err);alert("Não foi possível remover o EPI.")}}window.removeEpi=removeEpi;
async function removePlant(id){if(!canDeleteOperational())return;if(db.employees.some(e=>e.plantId===id)||db.epis.some(p=>p.plantId===id)){alert("Esta usina possui colaboradores ou EPIs vinculados. Remova ou transfira esses cadastros antes.");return}const name=plantName(id);try{await deleteDoc(doc(firestore,"plants",id));logAudit("REMOCAO_USINA",name);db.plants=db.plants.filter(x=>x.id!==id);refresh();showToast("Usina removida.")}catch(err){console.error(err);alert("Não foi possível remover a usina.")}}window.removePlant=removePlant;
async function removeUser(id){if(!isOwner())return;if(id===auth.currentUser.uid){alert("O proprietário não pode remover o próprio acesso enquanto estiver conectado.");return}const u=db.users.find(x=>x.id===id);try{await deleteDoc(doc(firestore,"users",id));logAudit("REMOCAO_USUARIO",u?.email||id);db.users=db.users.filter(x=>x.id!==id);renderUsers();showToast("Usuário removido.")}catch(err){console.error(err);alert("Não foi possível remover o usuário.")}}window.removeUser=removeUser;
async function removeAudit(id){if(!isOwner())return;try{await deleteDoc(doc(firestore,"auditLogs",id));db.audit=db.audit.filter(x=>x.id!==id);renderAudit();showToast("Registro de auditoria removido.")}catch(err){console.error(err);alert("Não foi possível remover o registro.")}}
...
$("companyLogoFile").addEventListener("change",async e=>{const f=e.target.files?.[0];if(!f)return;try{pendingLogoDataUrl=await resizeLogo(f);$("companyLogoPreview").src=pendingLogoDataUrl}catch(err){console.error(err);alert("Não foi possível processar a imagem.")}});$("removeCompanyLogo").addEventListener("click",()=>{pendingLogoDataUrl="";$("companyLogoFile").value="";$("companyLogoPreview").src="./logo-symbol.png"});$("companyForm").onsubmit=async e=>{e.preventDefault();try{const data={companyName:$("companyName").value.trim(),logoDataUrl:pendingLogoDataUrl||"",updatedAt:serverTimestamp(),updatedBy:auth.currentUser.uid};await setDoc(doc(firestore,"settings","company"),data,{merge:true});logAudit("CONFIGURACOES_EMPRESA",data.companyName);db.settings={...db.settings,...data};applyBranding();renderSettings();showToast("Configurações salvas.")}catch(err){console.error(err);alert("Não foi possível salvar as configurações.")}};
...
function buildNotifications(){
  const list=[];
  db.epis.filter(p=>stockQty(p)===0).forEach(p=>list.push({type:"critical",title:`${p.name} sem estoque`,text:plantName(p.plantId),action:`goToEpi('${p.plantId}')`,time:0}));
  db.epis.filter(p=>epiStockState(p).key==="low").forEach(p=>list.push({type:"warning",title:`${p.name} em estoque mínimo`,text:`${plantName(p.plantId)} • ${stockQty(p)} unidade(s)`,action:`goToEpi('${p.plantId}')`,time:0}));
  db.movements.filter(m=>(m.signatureStatus||"PENDENTE")!=="ASSINADO").forEach(m=>list.push({type:"info",title:"Assinatura pendente",text:`${emp(m.employeeId)?.name||"Colaborador"} • ${epi(m.epiId)?.name||"EPI"}`,action:`openPendingSignatures()`,time:new Date(fmtRaw(m.date)).getTime()}));
  return list.sort((a,b)=>({critical:0,warning:1,info:2}[a.type]-({critical:0,warning:1,info:2}[b.type])||b.time-a.time));
}
function renderNotifications(){
  const list=buildNotifications(),critical=list.filter(x=>x.type==="critical").length,warning=list.filter(x=>x.type==="warning").length,info=list.filter(x=>x.type==="info").length,total=list.length;
  ...
  if($("notificationList"))$("notificationList").innerHTML=list.map(n=>`<button class="notification-item ${n.type}" onclick="${n.action};closeNotificationPanel()"><span class="notification-type-dot"></span><div><strong>${esc(n.title)}</strong><small>${esc(n.text)}</small></div><span class="notification-arrow">›</span></button>`).join("")||'<div class="notification-empty">Tudo certo por aqui.<br><small>Nenhuma notificação ativa.</small></div>';
}
