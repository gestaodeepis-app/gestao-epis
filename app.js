...
function renderDashboard(){
  ...
  const scopedMoves=db.movements.filter(x=>!plant||x.plantId===plant);
  const scopedEpis=db.epis.filter(p=>!plant||p.plantId===plant);
  const now=new Date(),m=now.getMonth(),y=now.getFullYear();
  const month=scopedMoves.filter(x=>{const d=new Date(fmtRaw(x.date));return d.getMonth()===m&&d.getFullYear()===y});
  ...
  const stock=scopedEpis.reduce((a,p)=>a+stockQty(p),0);
  const pend=scopedMoves.filter(x=>(x.signatureStatus||"PENDENTE")!=="ASSINADO").length;
  const low=scopedEpis.filter(p=>["zero","low"].includes(epiStockState(p).key)).length;

  const kpis=[
    {icon:"↗",label:"Entregues no mês",value:ent,detail:"Quantidade entregue"},
    {icon:"↙",label:"Devoluções no mês",value:dev,detail:"Quantidade devolvida"},
    {icon:"▣",label:"Itens em estoque",value:stock,detail:`${low} item(ns) em atenção`},
    {icon:"✓",label:"Assinaturas pendentes",value:pend,detail:"Clique para visualizar",click:true}
  ];
  $("kpis").innerHTML=kpis.map(x=>x.click
    ? `<button class="kpi kpi-clickable" onclick="openPendingSignatures()"><div class="kpi-top"><span class="kpi-icon">${x.icon}</span><span>${x.label}</span></div><strong>${x.value}</strong><small>${x.detail}</small></button>`
    : `<div class="kpi"><div class="kpi-top"><span class="kpi-icon">${x.icon}</span><span>${x.label}</span></div><strong>${x.value}</strong><small>${x.detail}</small></div>`
  ).join("");

  if($("quickActions"))$("quickActions").innerHTML=isSSMA()?`
    <span class="quick-actions-title">Ações rápidas</span>
    <button onclick="openMovementModalPreset('Entrega')"><b>＋</b> Registrar entrega</button>
    <button onclick="openMovementModalPreset('Devolução')"><b>↩</b> Registrar devolução</button>
    <button onclick="openEmployeeModal()"><b>♙</b> Novo colaborador</button>
    <button onclick="openEpiModal()"><b>◇</b> Novo EPI</button>`:"";

  const alerts=[];
  scopedEpis.filter(p=>epiStockState(p).key==="zero").slice(0,5).forEach(p=>alerts.push(`<button class="alert alert-button" onclick="goToEpi('${p.plantId}')"><span class="pill danger">Sem estoque</span><div><strong>${esc(p.name)}</strong><span>${esc(plantName(p.plantId))}</span></div></button>`));
  scopedEpis.filter(p=>epiStockState(p).key==="low").slice(0,5).forEach(p=>alerts.push(`<button class="alert alert-button" onclick="goToEpi('${p.plantId}')"><span class="pill warning">Estoque baixo</span><div><strong>${esc(p.name)}</strong><span>${stockQty(p)} disponível(is) • mínimo ${epiMin(p)}</span></div></button>`));
...
function renderEmployees(){
  ...
  const list=db.employees.filter(e=>(e.name||"").toLowerCase().includes(q)||(e.reg||"").toLowerCase().includes(q));
  const cardTarget=$("employeeCards");
  const tableTarget=$("employeesTable");

  if(cardTarget){
    cardTarget.innerHTML=list.map(e=>{
      const moves=db.movements.filter(m=>m.employeeId===e.id);
      ...
      const initials=(e.name||"?").split(/\s+/).slice(0,2).map(x=>x[0]).join("").toUpperCase();
      return `<article class="entity-card employee-card" onclick="openEmployeeHistory('${e.id}')">
        <div class="entity-card-head"><div class="employee-avatar">${esc(initials)}</div><div><h4>${esc(e.name)}</h4><p>${esc(e.role||"-")} • ${esc(plantName(e.plantId))}</p></div></div>
        <div class="entity-card-meta"><span>Matrícula <strong>${esc(e.reg||"-")}</strong></span><span>EPIs entregues <strong>${delivered}</strong></span></div>
        <div class="entity-card-footer"><span class="status ${pending?"danger":"ok"}">${pending?`${pending} pendência(s)`:"Sem pendências"}</span><div class="card-actions" onclick="event.stopPropagation()"><button class="table-action" onclick="openEmployeeHistory('${e.id}')">Abrir ficha</button>${canDeleteOperational()?`<button class="table-action danger-btn" onclick="removeEmployee('${e.id}')">Remover</button>`:""}</div></div>
      </article>`;
    }).join("")||'<div class="empty-state">Nenhum colaborador cadastrado.</div>';
    return;
  }

  // Fallback for an older cached index.html.
  if(tableTarget){
    tableTarget.innerHTML=list.map(e=>{
      ...
      return `<tr><td><button class="employee-name-link" onclick="openEmployeeHistory('${e.id}')">${esc(e.name)}</button></td><td>${esc(e.reg||"-")}</td><td>${esc(e.role||"-")}</td><td>${esc(plantName(e.plantId))}</td><td><span class="status ok">${esc(e.status||"Ativo")}</span></td><td>${last?fmtDate(last.date):"Sem registro"}</td><td><button class="table-action" onclick="openEmployeeHistory('${e.id}')">Histórico</button></td></tr>`;
...
function renderEpis(){
  const plantFilter=$("epiPlantFilter")?.value||"";
  const list=[...db.epis].filter(p=>!plantFilter||p.plantId===plantFilter).sort((a,b)=>(plantName(a.plantId)+a.name).localeCompare(plantName(b.plantId)+b.name));
  const cardTarget=$("epiCards");
  const tableTarget=$("episTable");

  if(cardTarget){
    cardTarget.innerHTML=list.map(p=>{
      const state=epiStockState(p), qty=stockQty(p), min=epiMin(p);
      const target=Math.max(min*2,qty,1);
      const pct=Math.max(0,Math.min(100,Math.round((qty/target)*100)));
      return `<article class="entity-card epi-card">
        <div class="epi-card-top"><div><span class="eyebrow">${esc(plantName(p.plantId))}</span><h4>${esc(p.name||"-")}</h4><p>C.A. ${esc(p.ca||"-")} • ${esc(p.unit||"-")}</p></div><span class="status stock-${state.key}">${state.label}</span></div>
        <div class="epi-stock-row"><strong>${qty}</strong><span>em estoque</span><small>Mínimo: ${min}</small></div>
        <div class="stock-progress"><span style="width:${pct}%"></span></div>
        <div class="epi-sizes">Tamanhos: <strong>${esc((p.sizes||[]).join(", ")||"Único")}</strong></div>
        <div class="entity-card-footer"><span></span><div class="card-actions">${isSSMA()?`<button class="table-action" onclick="openEpiModal('${p.id}')">Editar / Reabastecer</button>`:""}${canDeleteOperational()?`<button class="table-action danger-btn" onclick="removeEpi('${p.id}')">Remover</button>`:""}</div></div>
      </article>`;
  ...
  if(tableTarget){
    tableTarget.innerHTML=list.map(p=>`<tr><td>${esc(plantName(p.plantId))}</td><td>${esc(p.name||"-")}</td><td>${esc(p.ca||"-")}</td><td>${esc(p.unit||"-")}</td><td>${esc((p.sizes||[]).join(", ")||"Único")}</td><td>${stockQty(p)}</td><td>${isSSMA()?`<button class="table-action" onclick="openEpiModal('${p.id}')">Editar</button>`:"-"}</td></tr>`).join("")||'<tr><td colspan="7" class="empty-state">Nenhum EPI cadastrado.</td></tr>';
  }
}
