...
async function removeSelectedAudits(){
  ...
  if(!confirm(`Remover ${ids.length} registro(s) selecionado(s) da auditoria?`))return;
...
async function removeAllAudits(){
  ...
  if(!confirm(`Remover TODOS os ${db.audit.length} registros do log de auditoria? Esta ação não poderá ser desfeita.`))return;
...
async function removeMovement(id){if(!canDeleteOperational())return;if(!confirm("Remover esta movimentação? O estoque será ajustado automaticamente."))return;const moveRef=doc(firestore,"movements",id);try{await runTransaction(firestore,async tx=>{const ms=await tx.get(moveRef);if(!ms.exists())return;const m=ms.data(),er=doc(firestore,"epis",m.epiId),es=await tx.get(er);if(es.exists()){const current=stockQty(es.data()),qty=+m.qty||0;let next=current;if(m.type==="Entrega")next=current+qty;else if(m.type==="Devolução"&&(m.returnDisposition||"REESTOQUE")==="REESTOQUE"){if(current<qty)throw new Error("ESTOQUE_INCONSISTENTE");next=current-qty}if(next!==current)tx.update(er,{stockQty:next,updatedAt:serverTimestamp(),updatedBy:auth.currentUser.uid})}tx.delete(moveRef)});logAudit("REMOCAO_MOVIMENTACAO",id);await reloadOperationalData()}catch(err){console.error(err);alert(err.message==="ESTOQUE_INCONSISTENTE"?"Não é possível remover esta devolução porque parte desse estoque já foi utilizado.":"Não foi possível remover a movimentação.")}}window.removeMovement=removeMovement;
async function removeEmployee(id){if(!canDeleteOperational())return;if(db.movements.some(m=>m.employeeId===id)){alert("Este colaborador possui movimentações. Remova as movimentações relacionadas antes.");return}const e=emp(id);if(!confirm(`Remover ${e?.name||"este colaborador"}?`))return;try{await deleteDoc(doc(firestore,"employees",id));logAudit("REMOCAO_COLABORADOR",e?.name||id);db.employees=db.employees.filter(x=>x.id!==id);refresh();showToast("Colaborador removido.")}catch(err){console.error(err);alert("Não foi possível remover o colaborador.")}}window.removeEmployee=removeEmployee;
async function removeEpi(id){if(!canDeleteOperational())return;if(db.movements.some(m=>m.epiId===id)){alert("Este EPI possui movimentações. Remova as movimentações relacionadas antes.");return}const p=epi(id);if(!confirm(`Remover ${p?.name||"este EPI"}?`))return;try{await deleteDoc(doc(firestore,"epis",id));logAudit("REMOCAO_EPI",p?.name||id);db.epis=db.epis.filter(x=>x.id!==id);refresh();showToast("EPI removido.")}catch(err){console.error(err);alert("Não foi possível remover o EPI.")}}window.removeEpi=removeEpi;
async function removePlant(id){if(!canDeleteOperational())return;if(db.employees.some(e=>e.plantId===id)||db.epis.some(p=>p.plantId===id)){alert("Esta usina possui colaboradores ou EPIs vinculados. Remova ou transfira esses cadastros antes.");return}const name=plantName(id);if(!confirm(`Remover a usina ${name}?`))return;try{await deleteDoc(doc(firestore,"plants",id));logAudit("REMOCAO_USINA",name);db.plants=db.plants.filter(x=>x.id!==id);refresh();showToast("Usina removida.")}catch(err){console.error(err);alert("Não foi possível remover a usina.")}}window.removePlant=removePlant;
async function removeUser(id){if(!isOwner())return;if(id===auth.currentUser.uid){alert("O proprietário não pode remover o próprio acesso enquanto estiver conectado.");return}const u=db.users.find(x=>x.id===id);if(!confirm(`Remover o acesso de ${u?.name||u?.email||"este usuário"}?`))return;try{await deleteDoc(doc(firestore,"users",id));logAudit("REMOCAO_USUARIO",u?.email||id);db.users=db.users.filter(x=>x.id!==id);renderUsers();showToast("Usuário removido.")}catch(err){console.error(err);alert("Não foi possível remover o usuário.")}}window.removeUser=removeUser;
