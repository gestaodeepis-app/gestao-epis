...
const isAdmin=()=>isOwner()||isAdministrator();
...
const normalizedPlantIds=u=>{
  if(!u)return [];
  if(Array.isArray(u.plantIds)&&u.plantIds.length)return [...new Set(u.plantIds.filter(Boolean))];
  if(u.plantId)return [u.plantId];
  return [];
};
const hasAllPlants=u=>isOwnerUser(u)||normalizedPlantIds(u).includes("TODAS");
const isOwnerUser=u=>u?.isOwner===true||u?.role==="PROPRIETARIO";
const allowedPlantIdsFor=u=>{
  if(!u)return [];
  if(isOwnerUser(u)||normalizedPlantIds(u).includes("TODAS"))return db.plants.map(p=>p.id);
  return normalizedPlantIds(u).filter(id=>id!=="TODAS");
};
const canAccessPlantId=(plantId,u=profile)=>{
  if(!u||!plantId)return false;
  return isOwnerUser(u)||normalizedPlantIds(u).includes("TODAS")||normalizedPlantIds(u).includes(plantId);
};
function formatUserPlants(u){
  if(!u)return "Usina não definida";
  if(isOwnerUser(u)||normalizedPlantIds(u).includes("TODAS"))return "Todas as usinas";
  const ids=normalizedPlantIds(u);
  if(!ids.length)return "Usina não definida";
  const names=ids.map(id=>plantName(id)).filter(x=>x&&x!=="-");
  return names.length?names.join(", "):`${ids.length} usina(s) autorizada(s)`;
}
...
let auditLoaded=false,auditLoading=false,notificationStateUnsubscribe=null,notificationCloudReadKeys=new Set(),manualSyncRunning=false;
...
let lastForegroundDataSync=0;
document.addEventListener("visibilitychange",()=>{
  if(document.visibilityState==="visible"){
    writePresence(false);
    if(profile&&navigator.onLine&&Date.now()-lastForegroundDataSync>120000){
      lastForegroundDataSync=Date.now();
      syncSystemData().catch(()=>{});
    }
  }
});
...
function closeTopActions(){
  const menu=$("topActionsMenu"),btn=$("topActionsBtn");
  if(menu)menu.classList.add("hidden");
  if(btn)btn.setAttribute("aria-expanded","false");
}
function toggleTopActions(event){
  event?.stopPropagation();
  const menu=$("topActionsMenu"),btn=$("topActionsBtn");if(!menu)return;
  const opening=menu.classList.contains("hidden");
  closeTopActions();
  if(opening){menu.classList.remove("hidden");btn?.setAttribute("aria-expanded","true")}
}
window.toggleTopActions=toggleTopActions;window.closeTopActions=closeTopActions;
document.addEventListener("click",e=>{if(!e.target.closest(".top-actions-wrap"))closeTopActions()});
document.addEventListener("keydown",e=>{if(e.key==="Escape")closeTopActions()});
function canonicalSystemUrl(){
  return `${location.origin}${location.pathname.replace(/\/?$/,"/")}`;
}
async function shareSystem(){
  const url=canonicalSystemUrl();
  try{
    if(navigator.share){
    ...
    showToast("Link do sistema copiado.");
  }catch(err){
    if(err?.name!=="AbortError"){
      try{await navigator.clipboard.writeText(url);showToast("Link do sistema copiado.")}catch(_){showToast("Não foi possível compartilhar o link.","error")}
    }
  }
}
...
function showAuthLoading(message="Carregando seu ambiente..."){
  if($("authLoadingScreen"))$("authLoadingScreen").classList.remove("hidden");
  const text=document.querySelector("#authLoadingScreen .auth-loading-card span");if(text)text.textContent=message;
  $("loginScreen")?.classList.add("hidden");$("accessPendingScreen")?.classList.add("hidden");
}
function hideAuthLoading(){$("authLoadingScreen")?.classList.add("hidden")}
...
function resetLoggedOutUi(){
  stopNotificationStateSync();hideAuthLoading();setAuthenticatedUi(false);
  profile=null;
...
function showPendingAccess(data,state="PENDENTE"){
  hideAuthLoading();setAuthenticatedUi(false);$("loginScreen").classList.add("hidden");$("accessPendingScreen").classList.remove("hidden");$("pendingUserName").textContent=data?.name||"Usuário";$("pendingUserEmail").textContent=data?.email||auth.currentUser?.email||"-";$("pendingEnterBtn").classList.add("hidden");
...
$("registerForm").addEventListener("submit",async e=>{e.preventDefault();$("registerError").classList.remove("show");const name=$("registerName").value.trim(),jobTitle=$("registerJobTitle").value.trim(),email=$("registerEmail").value.trim().toLowerCase(),p=$("registerPassword").value,p2=$("registerPassword2").value;if(name.length<3){showRegisterError("Informe seu nome completo.");return}if(jobTitle.length<2){showRegisterError("Informe sua função na empresa.");return}if(p!==p2){showRegisterError("As senhas não coincidem.");return}registrationInProgress=true;try{const cred=await createUserWithEmailAndPassword(auth,email,p);const data={name,jobTitle,email,status:"PENDENTE",role:"",plantId:"",plantIds:[],isOwner:false,photoUrl:"",createdAt:serverTimestamp(),requestedAt:serverTimestamp(),hierarchyVersion:1};await setDoc(doc(firestore,"users",cred.user.uid),data);registrationInProgress=false;$("registerForm").reset();showPendingAccess({...data,uid:cred.user.uid,email},"PENDENTE")}catch(err){registrationInProgress=false;console.error(err);showRegisterError(firebaseAuthMessage(err));if(auth.currentUser)await signOut(auth).catch(()=>{})}});
...
onAuthStateChanged(auth,async user=>{
  ...
  try{showAuthLoading("Sincronizando seus dados...");await loadAll();startNotificationStateSync();hideAuthLoading();$("loginScreen").classList.add("hidden");setAuthenticatedUi(true);handleEmployeeDeepLink();handleStartupView()}catch(err){console.error("Falha ao carregar dados:",err);hideAuthLoading();$("loginScreen").classList.add("hidden");setAuthenticatedUi(true);showToast("Você continua conectado. Alguns dados não puderam ser atualizados agora.","error")}
});
function updateProfileUI(){
  if(!profile)return;
  const displayName=profile.name||auth.currentUser?.email||"Usuário";
  const publicFunction=profile.jobTitle||((isOwner())?"Proprietário":"Função não informada");
  const accessLabel=isOwner()?"Todas as usinas":formatUserPlants(profile);
  $("userName").textContent=displayName;$("userRole").textContent=publicFunction;$("userPlant").textContent=accessLabel;
  if($("topUserName"))$("topUserName").textContent=displayName;
  if($("topUserRole"))$("topUserRole").textContent=`${publicFunction}${!isOwner()?` • ${accessLabel}`:""}`;
...
function assignedPlantIds(){
  if(!profile||isOwner())return [];
  const ids=normalizedPlantIds(profile);
  if(ids.includes("TODAS"))return [];
  return ids;
}
function assignedPlantId(){
  const ids=assignedPlantIds();
  return ids.length===1?ids[0]:"";
}
function hasRestrictedPlants(){
  return !!profile&&!isOwner()&&!normalizedPlantIds(profile).includes("TODAS");
}
async function readPlantsForProfile(){
  if(!hasRestrictedPlants())return readCollection("plants");
  const ids=assignedPlantIds();
  const rows=await Promise.all(ids.map(id=>getDoc(doc(firestore,"plants",id))));
  return rows.filter(s=>s.exists()).map(s=>({id:s.id,...s.data()}));
}
async function readScopedCollection(name){
  if(!hasRestrictedPlants())return readCollection(name);
  const ids=assignedPlantIds();
  const groups=await Promise.all(ids.map(id=>getDocs(query(collection(firestore,name),where("plantId","==",id)))));
  return groups.flatMap(s=>s.docs.map(d=>({id:d.id,...d.data()})));
}
function enforceAssignedPlant(){
  const ids=assignedPlantIds();
  if(ids.length===1)dashboardPlantId=ids[0];
  else if(dashboardPlantId&&!canAccessPlantId(dashboardPlantId))dashboardPlantId="";
}
...
async function readRecentMovements(){
  ...
  if(hasRestrictedPlants()){
    const ids=assignedPlantIds();
    const groups=await Promise.all(ids.map(id=>getDocs(query(collection(firestore,"movements"),where("plantId","==",id)))));
    return groups.flatMap(s=>s.docs.map(d=>({id:d.id,...d.data()})))
      .filter(m=>new Date(fmtRaw(m.date))>=cutoff)
      .sort((a,b)=>new Date(fmtRaw(b.date))-new Date(fmtRaw(a.date))).slice(0,1500);
  }
...
async function ensureAuditLoaded(force=false){
  if(!isOwner()||auditLoading||(!force&&auditLoaded))return;
  auditLoading=true;
...
async function syncSystemData(){
  ...
  if(!navigator.onLine){showToast("Sem conexão. Conecte-se à internet para sincronizar.","error");return}
  manualSyncRunning=true;
  const btn=$("syncDataBtn");
  if(btn){btn.disabled=true;btn.classList.add("is-syncing")}
  const started=Date.now();
  showToast("Sincronizando dados com a nuvem...");
  try{
    const currentView=document.querySelector(".view.active")?.id||"dashboard";
    const tasks=[readPlantsForProfile(),readScopedCollection("employees"),readScopedCollection("epis"),readRecentMovements(),readSettings()];
    if(isOwner())tasks.push(readCollection("users"));
    const res=await Promise.all(tasks);
    db.plants=res[0];db.employees=res[1];db.epis=res[2];db.movements=res[3];db.settings=res[4]||{};
    if(isOwner())db.users=res[5]||[];
    pendingLogoDataUrl=db.settings.logoDataUrl||null;
    enforceAssignedPlant();applyBranding();updateProfileUI();refresh();
    startNotificationStateSync();
    if(currentView==="auditoria"&&isOwner())await ensureAuditLoaded(true);
    const elapsed=Math.max(0.3,(Date.now()-started)/1000).toFixed(1);
    showToast(`Dados sincronizados com sucesso em ${elapsed}s.`);
  }catch(err){
    console.error("Sincronização manual:",err);
    showToast("Não foi possível sincronizar todos os dados. Tente novamente.","error");
  }finally{
    manualSyncRunning=false;
    if(btn){btn.disabled=false;btn.classList.remove("is-syncing")}
  }
}
window.syncSystemData=syncSystemData;
...
function navTo(view){
  ...
  if($("dashboardPlantWrap"))$("dashboardPlantWrap").style.display="";
...
function closeMobileMenu(){document.querySelector(".sidebar").classList.remove("mobile-open");$("mobileMenuOverlay").classList.remove("show");document.body.classList.remove("menu-open");document.body.style.overflow="";document.body.style.touchAction=""}
...
function renderMovements(){
  ...
  const filtered=[...movementSourceRows()].sort((a,b)=>new Date(fmtRaw(b.date))-new Date(fmtRaw(a.date))).filter(x=>(!dashboardPlantId||x.plantId===dashboardPlantId)&&(!type||x.type===type)&&(!sig||(x.signatureStatus||"PENDENTE")===sig)&&(((emp(x.employeeId)?.name||"").toLowerCase().includes(q))||((epi(x.epiId)?.name||"").toLowerCase().includes(q))));
...
function employeesForCurrentPlant(){
  return db.employees.filter(e=>!dashboardPlantId||e.plantId===dashboardPlantId).sort((a,b)=>(a.name||"").localeCompare(b.name||"","pt-BR",{sensitivity:"base"}));
}
function printAllEmployeeQrs(){
  const rows=employeesForCurrentPlant();
  if(!rows.length){showToast("Nenhum colaborador disponível neste filtro.","error");return}
  const popup=window.open("","_blank");if(!popup){userAlert("O navegador bloqueou a janela de impressão.");return}
  const company=esc(db.settings.companyName||"Gestão de EPIs"),logo=db.settings.logoDataUrl?`<img class="company-logo" src="${db.settings.logoDataUrl}" alt="Logo">`:"";
  const labels=rows.map(e=>`<article class="qr-label">${logo}<img class="qr" src="${employeeQrUrl(e.id,500)}" alt="QR"><strong>${esc(e.name)}</strong><span>Matrícula ${esc(e.reg||"-")}</span><small>${esc(e.role||"-")} • ${esc(plantName(e.plantId))}</small></article>`).join("");
  popup.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>QR Codes - ${company}</title><style>@page{size:A4;margin:8mm}*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;color:#111}.header{display:flex;justify-content:space-between;align-items:end;border-bottom:1px solid #aaa;padding-bottom:5mm;margin-bottom:6mm}.header h1{font-size:17px;margin:0}.header p{margin:2mm 0 0;color:#555;font-size:10px}.sheet{display:grid;grid-template-columns:repeat(3,1fr);gap:5mm}.qr-label{break-inside:avoid;border:1px solid #c8c8c8;border-radius:8px;padding:5mm;text-align:center;min-height:82mm;display:flex;flex-direction:column;align-items:center}.company-logo{height:10mm;max-width:35mm;object-fit:contain;margin-bottom:2mm}.qr{width:43mm;height:43mm}.qr-label strong{font-size:10px;margin-top:3mm}.qr-label span,.qr-label small{font-size:8px;color:#555;margin-top:1mm}.foot{font-size:8px;color:#777;margin-top:6mm;text-align:center}@media print{button{display:none}}</style></head><body><div class="header"><div><h1>${company} — QR Codes dos colaboradores</h1><p>${dashboardPlantId?esc(plantName(dashboardPlantId)):"Todas as usinas"} • ${rows.length} colaborador(es)</p></div></div><div class="sheet">${labels}</div><div class="foot">Escaneie o QR para abrir a ficha de EPI do colaborador.</div><script>window.onload=()=>setTimeout(()=>window.print(),1200)<\/script></body></html>`);
  popup.document.close();
}
window.printAllEmployeeQrs=printAllEmployeeQrs;
...
function setEpiView(mode){
  epiView=mode;
  const cards=$("epiCards"),list=$("epiListWrap");
  $("epiCardsBtn").classList.toggle("active",mode==="cards");
  $("epiListBtn").classList.toggle("active",mode==="list");
  cards.classList.toggle("hidden",mode!=="cards");
  list.classList.toggle("hidden",mode!=="list");
  cards.style.display=mode==="cards"?"grid":"none";
  list.style.display=mode==="list"?"block":"none";
...
function renderEpis(){
  const plantFilter=dashboardPlantId||$("epiPlantFilter")?.value||"",list=[...db.epis].filter(p=>!plantFilter||p.plantId===plantFilter).sort((a,b)=>(plantName(a.plantId)+a.name).localeCompare(plantName(b.plantId)+b.name));
...
function setUserView(mode){
  userView=mode;
  const cards=$("userCards"),list=$("userListWrap");
  $("userCardsBtn").classList.toggle("active",mode==="cards");
  $("userListBtn").classList.toggle("active",mode==="list");
  cards.classList.toggle("hidden",mode!=="cards");
  list.classList.toggle("hidden",mode!=="list");
  cards.style.display=mode==="cards"?"grid":"none";
  list.style.display=mode==="list"?"block":"none";
...
function populateDashboardFilters(){
  const dashboard=$("dashboardPlantFilter"),mobile=$("mobileDashboardPlantFilter"),epiFilter=$("epiPlantFilter");if(!dashboard)return;
  const restricted=hasRestrictedPlants(),ids=assignedPlantIds(),single=restricted&&ids.length===1;
  const options='<option value="">'+(restricted?"Todas as minhas usinas":"Todas as usinas")+'</option>'+db.plants.map(p=>`<option value="${p.id}">${esc(p.name)}</option>`).join("");
  dashboard.innerHTML=options;
  if(mobile)mobile.innerHTML=options;
  if(epiFilter)epiFilter.innerHTML=options;

  if(single){
    dashboard.value=ids[0];dashboard.disabled=true;dashboardPlantId=ids[0];
    if(mobile){mobile.value=ids[0];mobile.disabled=true}
    if(epiFilter){epiFilter.value=ids[0];epiFilter.disabled=true}
    $("dashboardPlantWrap")?.classList.add("locked-plant");$("mobileDashboardPlantWrap")?.classList.add("locked-plant");
    return;
  }

  dashboard.disabled=false;if(mobile)mobile.disabled=false;if(epiFilter)epiFilter.disabled=false;
  if(dashboardPlantId&&!db.plants.some(p=>p.id===dashboardPlantId))dashboardPlantId="";
  dashboard.value=dashboardPlantId;
  if(mobile)mobile.value=dashboardPlantId;
  if(epiFilter)epiFilter.value=dashboardPlantId;
  $("dashboardPlantWrap")?.classList.remove("locked-plant");$("mobileDashboardPlantWrap")?.classList.remove("locked-plant");
}
function applyGlobalPlantFilter(value){
  const ids=assignedPlantIds();dashboardPlantId=(ids.length===1?ids[0]:(value&&canAccessPlantId(value)?value:""));
  if($("dashboardPlantFilter"))$("dashboardPlantFilter").value=dashboardPlantId;
  if($("mobileDashboardPlantFilter"))$("mobileDashboardPlantFilter").value=dashboardPlantId;
  if($("epiPlantFilter")&&assignedPlantIds().length!==1)$("epiPlantFilter").value=dashboardPlantId;
  if($("reportPlant")&&assignedPlantIds().length!==1)$("reportPlant").value=dashboardPlantId;
  movementPage=1;
  renderDashboard();renderMovements();renderEmployees();renderEpis();renderUsers();updateReportSummary();
}
window.applyGlobalPlantFilter=applyGlobalPlantFilter;
if($("dashboardPlantFilter"))$("dashboardPlantFilter").addEventListener("change",()=>applyGlobalPlantFilter($("dashboardPlantFilter").value));
if($("mobileDashboardPlantFilter"))$("mobileDashboardPlantFilter").addEventListener("change",()=>applyGlobalPlantFilter($("mobileDashboardPlantFilter").value));
function populateSelects(){
  const options=db.plants.map(p=>`<option value="${p.id}">${esc(p.name)}</option>`).join(""),ids=assignedPlantIds(),single=ids.length===1;
  $("ePlant").innerHTML=options;$("pPlant").innerHTML=options;
  if(single){$("ePlant").value=ids[0];$("pPlant").value=ids[0];$("ePlant").disabled=true;$("pPlant").disabled=true}
  else{$("ePlant").disabled=false;$("pPlant").disabled=false}
}
function positionMovementEmployeeSuggestions(){
  const box=$("mEmployeeSuggestBox"),input=$("mEmployeeSearch");if(!box||!input)return;
  const mobile=window.matchMedia("(max-width: 820px)").matches;
  if(!mobile){box.style.removeProperty("position");box.style.removeProperty("left");box.style.removeProperty("right");box.style.removeProperty("top");box.style.removeProperty("bottom");box.style.removeProperty("max-height");return}
  const vv=window.visualViewport;
  const keyboard=Math.max(0,window.innerHeight-((vv?.offsetTop||0)+(vv?.height||window.innerHeight)));
  box.style.position="fixed";box.style.left="12px";box.style.right="12px";box.style.top="auto";box.style.bottom=`${Math.max(10,keyboard+10)}px`;box.style.maxHeight=`${Math.max(150,Math.min(260,(vv?.height||window.innerHeight)*.38))}px`;
}
function renderMovementEmployeeSuggestions(){const box=$("mEmployeeSuggestBox"),input=$("mEmployeeSearch");if(!box||!input)return;const q=input.value.trim().toLowerCase();$("mEmployeeId").value="";if(!q){box.classList.add("hidden");box.innerHTML="";$("mPlant").value="";updateMovementPlantAndEpis(null);return}const matches=db.employees.filter(e=>e.status==="Ativo"&&(e.name||"").toLowerCase().includes(q)).sort((a,b)=>{const ap=(a.name||"").toLowerCase().startsWith(q)?0:1,bp=(b.name||"").toLowerCase().startsWith(q)?0:1;return ap-bp||(a.name||"").localeCompare(b.name||"")}).slice(0,8);box.innerHTML=matches.map(e=>`<button type="button" class="autocomplete-option" data-employee-id="${e.id}"><strong>${esc(e.name)}</strong><small>${esc(e.role||"-")} • ${esc(plantName(e.plantId))}</small></button>`).join("")||'<div class="autocomplete-empty">Nenhum colaborador encontrado.</div>';box.classList.remove("hidden");positionMovementEmployeeSuggestions();box.querySelectorAll("[data-employee-id]").forEach(btn=>btn.addEventListener("click",()=>selectMovementEmployee(btn.dataset.employeeId)))}
...
$("mEmployeeSearch").addEventListener("input",renderMovementEmployeeSuggestions);$("mEmployeeSearch").addEventListener("focus",()=>{setTimeout(()=>$("mEmployeeSearch").scrollIntoView({behavior:"smooth",block:"start"}),120);if($("mEmployeeSearch").value.trim())renderMovementEmployeeSuggestions()});window.visualViewport?.addEventListener("resize",()=>{if(!$("mEmployeeSuggestBox")?.classList.contains("hidden"))positionMovementEmployeeSuggestions()});window.visualViewport?.addEventListener("scroll",()=>{if(!$("mEmployeeSuggestBox")?.classList.contains("hidden"))positionMovementEmployeeSuggestions()});document.addEventListener("click",e=>{if(!e.target.closest(".autocomplete-field")&&$("mEmployeeSuggestBox"))$("mEmployeeSuggestBox").classList.add("hidden")});$("mEpi").addEventListener("change",updateMovementSizes);$("mType").addEventListener("change",()=>$("returnDispositionWrap").classList.toggle("hidden",$("mType").value!=="Devolução"));
...
function renderPlantAccessPicker(containerId,selected=[]){
  const box=$(containerId);if(!box)return;
  const selectedSet=new Set(Array.isArray(selected)?selected:(selected?[selected]:[]));
  const all=selectedSet.has("TODAS");
  box.innerHTML=`<label class="plant-access-option all-plants"><input type="checkbox" value="TODAS" ${all?"checked":""}><span><strong>Todas as usinas</strong><small>Acesso atual e às novas usinas cadastradas.</small></span></label>`+
    db.plants.map(p=>`<label class="plant-access-option"><input type="checkbox" value="${p.id}" ${!all&&selectedSet.has(p.id)?"checked":""}><span><strong>${esc(p.name)}</strong></span></label>`).join("");
  const checks=[...box.querySelectorAll('input[type="checkbox"]')],allCheck=checks.find(x=>x.value==="TODAS");
  checks.forEach(ch=>ch.addEventListener("change",()=>{
    if(ch.value==="TODAS"&&ch.checked)checks.filter(x=>x!==ch).forEach(x=>x.checked=false);
    if(ch.value!=="TODAS"&&ch.checked&&allCheck)allCheck.checked=false;
  }));
}
function selectedPlantAccess(containerId){
  const vals=[...($(containerId)?.querySelectorAll('input[type="checkbox"]:checked')||[])].map(x=>x.value);
  return vals.includes("TODAS")?["TODAS"]:vals;
}
function accessLabelFromIds(ids){
  if(ids.includes("TODAS"))return "Todas as usinas";
  return ids.map(id=>plantName(id)).filter(x=>x&&x!=="-").join(", ")||"Nenhuma usina";
}
function populateApprovalPlants(selected=[]){renderPlantAccessPicker("approvalPlants",selected)}

function openUserApproval(id,reactivate=false){if(!isOwner())return;const u=db.users.find(x=>x.id===id);if(!u)return;$("approvalUserId").value=id;$("approvalUserName").textContent=u.name||"Usuário";$("approvalUserEmail").textContent=u.email||"-";$("approvalAvatar").textContent=(u.name||u.email||"U").split(/[\s@]+/).filter(Boolean).slice(0,2).map(x=>x[0]).join("").toUpperCase();$("approvalRole").value="ADMINISTRADOR";$("approvalJobTitle").value=u.jobTitle||"";populateApprovalPlants(normalizedPlantIds(u));document.querySelector("#userApprovalModal .modal-head h3").textContent=reactivate?"Reativar administrador":"Aprovar administrador";$("userApprovalModal").classList.add("open")}window.openUserApproval=openUserApproval;
$("userApprovalForm").onsubmit=async e=>{
  e.preventDefault();if(!isOwner())return;
  const id=$("approvalUserId").value,u=db.users.find(x=>x.id===id),role="ADMINISTRADOR",
    jobTitle=$("approvalJobTitle").value.trim(),plantIds=selectedPlantAccess("approvalPlants");
  if(!u||!jobTitle||!plantIds.length){showToast("Informe a função na empresa e selecione ao menos uma usina.","error");return}
  try{
    const legacyPlantId=plantIds.includes("TODAS")?"TODAS":plantIds[0];
    const data={status:"ATIVO",role,jobTitle,plantIds,plantId:legacyPlantId,isOwner:false,approvedAt:serverTimestamp(),approvedBy:auth.currentUser.uid,updatedAt:serverTimestamp(),updatedBy:auth.currentUser.uid};
    await updateDoc(doc(firestore,"users",id),data);
    const i=db.users.findIndex(x=>x.id===id);if(i>=0)db.users[i]={...db.users[i],...data,approvedAt:new Date(),updatedAt:new Date()};
    const accessLabel=accessLabelFromIds(plantIds);
    await logAudit("APROVACAO_USUARIO",`${u.name||u.email} • ${jobTitle} • ${accessLabel}`);
    closeModal("userApprovalModal");userStatusFilter="";if($("userStatusFilter"))$("userStatusFilter").value="";
    renderUsers();renderNotifications();renderPendingUsersBadge();
    showToast(`Acesso de ${u.name||u.email} aprovado: ${accessLabel}.`);
  }catch(err){console.error(err);showToast("Não foi possível aprovar o usuário.","error")}
};await updateDoc(doc(firestore,"users",id),data);const i=db.users.findIndex(x=>x.id===id);if(i>=0)db.users[i]={...db.users[i],...data,approvedAt:new Date(),updatedAt:new Date()};await logAudit("APROVACAO_USUARIO",`${u.name||u.email} • ${jobTitle} • ${plantName(plantId)}`);closeModal("userApprovalModal");userStatusFilter="";if($("userStatusFilter"))$("userStatusFilter").value="";renderUsers();renderNotifications();renderPendingUsersBadge();showToast(`Acesso de ${u.name||u.email} aprovado para ${plantName(plantId)}.`)}catch(err){console.error(err);showToast("Não foi possível aprovar o usuário.","error")}};
...
function editableUserById(id){
  if(id===auth.currentUser?.uid)return {id:auth.currentUser.uid,email:auth.currentUser.email,...profile};
  return db.users.find(x=>x.id===id)||null;
}
function openUserEdit(id){
  const u=editableUserById(id);if(!u)return;
  const own=id===auth.currentUser?.uid;
  if(!own&&!isOwner())return;
  $("editUserId").value=id;$("editUserName").value=u.name||"";$("editUserJobTitle").value=u.jobTitle||"";
  pendingUserPhotoDataUrl=u.photoUrl||"";renderUserPhotoPreview(u);$("editUserPhotoFile").value="";
  const ownerFields=$("ownerAccessFields");
  if(ownerFields){ownerFields.classList.toggle("hidden",own||!isOwner());}
  if(!own&&isOwner()){
    $("editUserAccessRole").value="ADMINISTRADOR";
    renderPlantAccessPicker("editUserPlants",normalizedPlantIds(u));
  }
  if($("userEditModalTitle"))$("userEditModalTitle").textContent=own?"Meu perfil":"Editar usuário";
  if($("userEditModalSubtitle"))$("userEditModalSubtitle").textContent=own?"Atualize seu nome, função e foto.":"Atualize dados profissionais e as usinas autorizadas deste administrador.";
  if($("userEditKicker"))$("userEditKicker").textContent=own?"MEU PERFIL":"ADMINISTRAÇÃO";
  $("userEditModal").classList.add("open");
}
window.openUserEdit=openUserEdit;
function openMyProfile(){if(!auth.currentUser||!profile)return;openUserEdit(auth.currentUser.uid)}
...
$("editUserPhotoFile").addEventListener("change",async e=>{const f=e.target.files?.[0];if(!f)return;try{pendingUserPhotoDataUrl=await resizeUserPhoto(f);renderUserPhotoPreview(editableUserById($("editUserId").value))}catch(err){console.error(err);userAlert("Não foi possível processar a foto.")}});
$("removeUserPhoto").addEventListener("click",()=>{pendingUserPhotoDataUrl="";renderUserPhotoPreview(editableUserById($("editUserId").value))});
$("userEditForm").onsubmit=async e=>{
  e.preventDefault();
  try{
    const id=$("editUserId").value,name=$("editUserName").value.trim(),jobTitle=$("editUserJobTitle").value.trim();
    const own=id===auth.currentUser?.uid;
    if(!own&&!isOwner()){userAlert("Você só pode editar o próprio perfil.");return}
    if(name.length<3||jobTitle.length<2){userAlert("Informe nome e função na empresa.");return}
    const data={name,jobTitle,photoUrl:pendingUserPhotoDataUrl||"",updatedAt:serverTimestamp(),updatedBy:auth.currentUser.uid};
    if(!own&&isOwner()){
      const plantIds=selectedPlantAccess("editUserPlants");
      if(!plantIds.length){userAlert("Selecione ao menos uma usina para este administrador.");return}
      data.role="ADMINISTRADOR";data.plantIds=plantIds;data.plantId=plantIds.includes("TODAS")?"TODAS":plantIds[0];data.isOwner=false;
    }
    await updateDoc(doc(firestore,"users",id),data);
    if(own){profile={...profile,...data,name,jobTitle,photoUrl:pendingUserPhotoDataUrl||""};updateProfileUI()}
    const i=db.users.findIndex(x=>x.id===id);if(i>=0)db.users[i]={...db.users[i],...data,name,jobTitle,photoUrl:pendingUserPhotoDataUrl||""};
    await logAudit(own?"EDICAO_PROPRIO_PERFIL":"EDICAO_USUARIO",`${name} • ${jobTitle}${data.plantIds?` • ${accessLabelFromIds(data.plantIds)}`:""}`);
    closeModal("userEditModal");renderUsers();showToast(own?"Perfil atualizado com sucesso.":"Usuário atualizado.");
  }catch(err){console.error(err);userAlert("Não foi possível atualizar o perfil.")}
};
...
function populateReportFilters(){
  const currentPlant=$("reportPlant").value,ids=assignedPlantIds(),single=ids.length===1;
...
function filteredReportMoves(showAlert=false){
  resolveReportEmployee(false);const typed=$("reportEmployeeSearch").value.trim(),employeeId=$("reportEmployeeId").value,plantId=$("reportPlant").value||dashboardPlantId,from=$("reportFrom").value?new Date($("reportFrom").value+"T00:00:00"):null,to=$("reportTo").value?new Date($("reportTo").value+"T23:59:59"):null;
...
function clearReportFilters(){$("reportEmployeeSearch").value="";$("reportEmployeeId").value="";$("reportPlant").disabled=assignedPlantIds().length===1;$("reportPlant").value=assignedPlantIds().length===1?assignedPlantIds()[0]:"";$("reportFrom").value="";$("reportTo").value="";updateReportArchiveHint();updateReportSummary()}window.clearReportFilters=clearReportFilters;
...
function localNotificationReadSet(){try{return new Set(JSON.parse(localStorage.getItem(notificationStorageKey())||"[]"))}catch{return new Set()}}
function notificationReadSet(){
  const merged=localNotificationReadSet();
  notificationCloudReadKeys.forEach(k=>merged.add(k));
  return merged;
}
function saveNotificationReadSet(set){localStorage.setItem(notificationStorageKey(),JSON.stringify([...set].slice(-500)))}
function stopNotificationStateSync(){
  if(notificationStateUnsubscribe){notificationStateUnsubscribe();notificationStateUnsubscribe=null}
  notificationCloudReadKeys=new Set();
}
function startNotificationStateSync(){
  stopNotificationStateSync();
  if(!auth.currentUser||!profile)return;
  const ref=doc(firestore,"notificationStates",auth.currentUser.uid);
  notificationStateUnsubscribe=onSnapshot(ref,snap=>{
    const cloud=snap.exists()&&Array.isArray(snap.data()?.readKeys)?snap.data().readKeys:[];
    notificationCloudReadKeys=new Set(cloud);
    const merged=notificationReadSet();
    saveNotificationReadSet(merged);
    renderNotifications();
  },err=>{
    console.debug("Sincronização das notificações:",err);
    renderNotifications();
  });
}
async function persistNotificationReadSet(set){
  const normalized=[...set].slice(-500);
  saveNotificationReadSet(new Set(normalized));
  notificationCloudReadKeys=new Set(normalized);
  renderNotifications();
  if(!auth.currentUser||!profile)return;
  try{
    await setDoc(doc(firestore,"notificationStates",auth.currentUser.uid),{
      readKeys:normalized,
      updatedAt:serverTimestamp()
    },{merge:true});
  }catch(err){
    console.error("Status das notificações:",err);
    showToast("A notificação foi marcada neste aparelho, mas ainda não sincronizou com a nuvem.","error");
  }
}
function notificationStamp(v){const raw=fmtRaw(v);return raw?new Date(raw).getTime():0}
function buildNotifications(){
  const list=[];
  db.epis.filter(p=>stockQty(p)===0).forEach(p=>list.push({
    key:`stock-zero:${p.id}:${notificationStamp(p.updatedAt)}`,type:"critical",
    title:`${p.name} sem estoque`,text:plantName(p.plantId),
    target:{kind:"epi",epiId:p.id,plantId:p.plantId},time:notificationStamp(p.updatedAt)
  }));
  db.epis.filter(p=>epiStockState(p).key==="low").forEach(p=>list.push({
    key:`stock-low:${p.id}:${stockQty(p)}:${notificationStamp(p.updatedAt)}`,type:"warning",
    title:`${p.name} em estoque mínimo`,text:`${plantName(p.plantId)} • ${stockQty(p)} unidade(s)`,
    target:{kind:"epi",epiId:p.id,plantId:p.plantId},time:notificationStamp(p.updatedAt)
  }));
  db.movements.filter(m=>(m.signatureStatus||"PENDENTE")!=="ASSINADO").forEach(m=>list.push({
    key:`signature:${m.id}:${notificationStamp(m.signatureUpdatedAt||m.updatedAt||m.date)}`,type:"info",
    title:"Assinatura pendente",text:`${emp(m.employeeId)?.name||"Colaborador"} • ${epi(m.epiId)?.name||"EPI"}`,
    target:{kind:"signature",movementId:m.id},time:new Date(fmtRaw(m.date)).getTime()
  }));
  if(isOwner())db.users.filter(u=>u.status==="PENDENTE").forEach(u=>list.push({
    key:`user-pending:${u.id}:${notificationStamp(u.requestedAt||u.createdAt)}`,type:"warning",
    title:"Novo usuário aguardando aprovação",text:`${u.name||u.email} • definir função e usina`,
    target:{kind:"userApproval",userId:u.id},time:notificationStamp(u.requestedAt||u.createdAt)
  }));
...
async function markNotificationRead(key){
  const set=notificationReadSet();set.add(key);
  await persistNotificationReadSet(set);
  showToast("Notificação marcada como lida.");
}
window.markNotificationRead=markNotificationRead;
async function markAllNotificationsRead(){
  const set=notificationReadSet();buildNotifications().forEach(n=>set.add(n.key));
  await persistNotificationReadSet(set);
  showToast("Todas as notificações foram marcadas como lidas.");
}
window.markAllNotificationsRead=markAllNotificationsRead;
async function openNotificationTarget(key){
  const item=buildNotifications().find(n=>n.key===key);
  if(!item)return;
  if(!item.read){
    const set=notificationReadSet();set.add(item.key);
    await persistNotificationReadSet(set);
  }
  closeNotificationPanel();
  const target=item.target||{};
  if(target.kind==="epi"){
    if(target.plantId&&canAccessPlantId(target.plantId))applyGlobalPlantFilter(target.plantId);
    navTo("epis");
    if($("epiPlantFilter"))$("epiPlantFilter").value=target.plantId||"";
    renderEpis();
    setTimeout(()=>{
      const el=document.querySelector(`[data-epi-id="${CSS.escape(target.epiId||"")}"]`);
      if(el){el.scrollIntoView({behavior:"smooth",block:"center"});el.classList.add("notification-focus");setTimeout(()=>el.classList.remove("notification-focus"),2600)}
    },120);
    return;
  }
  if(target.kind==="signature"){
    navTo("movimentacoes");
    if($("movSignature"))$("movSignature").value="PENDENTE";
    movementPage=1;renderMovements();
    const m=db.movements.find(x=>x.id===target.movementId);
    if(m)setTimeout(()=>openSignatureModal(m.id,m.signatureStatus||"PENDENTE"),100);
    return;
  }
  if(target.kind==="userApproval"&&isOwner()){
    // Usuário pendente ainda não tem usina definida; ele deve aparecer independente do filtro global.
    navTo("usuarios");setUserStatusFilter("PENDENTE");
    setTimeout(()=>openUserApproval(target.userId),100);
  }
}
...
function renderQrEmployeeSuggestions(){
  ...
  const rows=db.employees.filter(e=>(!dashboardPlantId||e.plantId===dashboardPlantId)&&(e.name||"").toLowerCase().includes(q)).sort((a,b)=>(a.name||"").localeCompare(b.name||"")).slice(0,6);
