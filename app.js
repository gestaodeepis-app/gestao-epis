...
function updatePwaInstallUI(){
  ...
  if($("installAppBtn"))$("installAppBtn").classList.toggle("hidden",installed||!deferredInstallPrompt);
  if($("settingsInstallBtn"))$("settingsInstallBtn").classList.toggle("hidden",installed||!deferredInstallPrompt);
  if($("pwaInstallStatusText"))$("pwaInstallStatusText").textContent=installed?"Instalado neste dispositivo":deferredInstallPrompt?"Pronto para instalar":"Abra pelo navegador para instalar";
...
async function installPwa(){
  ...
  if(!deferredInstallPrompt){showToast("A instalação não está disponível neste navegador agora.","error");return}
  deferredInstallPrompt.prompt();const result=await deferredInstallPrompt.userChoice;deferredInstallPrompt=null;updatePwaInstallUI();
...
onAuthStateChanged(auth,async user=>{
  ...
  try{await loadAll();$("loginScreen").classList.add("hidden");handleEmployeeDeepLink();handleStartupView()}catch(err){console.error("Falha ao carregar dados:",err);$("loginScreen").classList.add("hidden");showToast("Você continua conectado. Alguns dados não puderam ser atualizados agora.","error")}
});
...
function handleStartupView(){
  ...
  const allowed=["dashboard","movimentacoes","colaboradores","epis","relatorios","usuarios","auditoria","configuracoes"];
