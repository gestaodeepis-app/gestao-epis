...
$("logoutBtn").addEventListener("click",()=>{if(ownerUsersRefreshTimer)clearInterval(ownerUsersRefreshTimer);if(ownerUsersUnsubscribe){ownerUsersUnsubscribe();ownerUsersUnsubscribe=null}stopPendingWatcher();signOut(auth)});
...
async function loadAll(){
  const tasks=[readCollection("plants"),readCollection("employees"),readCollection("epis"),readRecentMovements(),readSettings()];
...
async function reloadOperationalData(){
  const [epis,movements]=await Promise.all([readCollection("epis"),readRecentMovements()]);
...
function clearReportFilters(){$("reportEmployeeSearch").value="";$("reportEmployeeId").value="";$("reportPlant").disabled=false;$("reportPlant").value="";$("reportFrom").value="";$("reportTo").value="";updateReportArchiveHint();updateReportSummary()}window.clearReportFilters=clearReportFilters;
