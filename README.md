...
## v2.1.1 — Sincronização entre dispositivos
- Status de notificações lidas passa a ser salvo no Firestore por usuário.
- Ao marcar uma notificação como lida no celular, PC e PWA recebem a alteração em tempo real.
- Mantido fallback local para continuar funcionando temporariamente sem conexão.
- Novo botão profissional “Sincronizar dados” no menu de ações superior.
- A sincronização manual atualiza usinas, colaboradores, EPIs, movimentações, configurações e usuários quando permitido.
- Ao retornar ao aplicativo após algum tempo, o sistema pode atualizar os dados automaticamente.
- Incluídas novas Firestore Rules para `notificationStates/{uid}`.

## v2.1.2 — Notificações, Perfil e Administração Mobile
- Corrigido erro `navTo is not defined` ao clicar em notificações.
- Notificações agora possuem destino estruturado e abrem diretamente o item relacionado.
- Alerta de estoque abre a aba de EPIs, aplica a usina correta e destaca o EPI.
- Assinatura pendente abre diretamente a movimentação para atualização da assinatura.
- Novo usuário pendente abre diretamente o modal de aprovação para o Proprietário.
- Usuários pendentes permanecem visíveis ao Proprietário mesmo quando existe um filtro global de usina ativo.
- Corrigido layout dos cards de usuários para nomes completos/longos.
- Perfil superior passa a abrir o modal “Meu perfil” para todos os papéis.
- O cartão de perfil no rodapé do menu também abre “Meu perfil”, útil no celular.
- Todos os usuários podem alterar o próprio nome e foto; Proprietário continua podendo editar outros usuários.
- Corrigido layout mobile de aprovação, edição e ações dos usuários.
- Firestore Rules atualizadas para permitir que o usuário ativo altere o próprio campo `name`.

## v2.1.2 — Correção de sintaxe
- Corrigido `Unexpected token 'class'` no `renderEpis`.
- O erro impedia o sistema de concluir a tela de carregamento.

## v2.2.0 — Permissões por usina e experiência mobile
- Administrador agora é restrito à usina definida pelo Proprietário.
- Novos cadastros informam Nome + Função na empresa.
- Novas aprovações usam apenas o perfil Administrador da unidade; SSMA/Visualizador permanecem apenas como compatibilidade para usuários antigos.
- Proprietário pode alterar nome, função e usina de outro usuário pelo modal Editar usuário.
- Usuários comuns veem no próprio perfil apenas nome, função profissional e usina; o perfil de acesso fica administrativo.
- Configurações, Usuários e Auditoria ficam exclusivas do Proprietário.
- Corrigida pesquisa de colaborador no mobile: sugestões ficam acima do teclado e permanecem clicáveis.
- Após aprovar usuário por notificação, o filtro PENDENTE é limpo para a lista não desaparecer.

## v2.2.1 — Acesso multiusinas
- Administradores podem receber acesso a uma, várias ou todas as usinas.
- Novo campo `plantIds` no perfil do usuário.
- `plantId` é mantido como compatibilidade com versões anteriores.
- Proprietário seleciona as usinas por caixas de seleção na aprovação e edição.
- Opção “Todas as usinas” também concede acesso automático a novas usinas cadastradas.
- Administrador com várias usinas pode alternar o filtro global somente entre as unidades autorizadas.
- Administrador com uma única usina continua com o filtro bloqueado nela.
- Leituras do Firestore passam a buscar apenas as usinas autorizadas.
- Firestore Rules atualizadas com `hasPlantAccess`.

## v2.2.2 — Correção de carregamento
- Removido um trecho antigo duplicado do fluxo de aprovação de usuários.
- O trecho residual da v2.2.0 causava `Unexpected token '}'` no navegador.
- Mantido o novo modelo de acesso a uma, várias ou todas as usinas.
- Versão e cache do PWA incrementados para forçar o carregamento do JavaScript corrigido.
